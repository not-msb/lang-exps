    ...
    pub fn fromAst(allocator: Allocator, expr: Ast) Allocator.Error!PostAst {
        return switch (expr.node) {
            ...
            .Tuple => |exprs| {
                ...
                for (exprs) |e| {
                    const post_expr = try PostAst.fromAst(allocator, e);
                    post_exprs = try push(PostAst, allocator, post_exprs, post_expr);
                }
                return .{ .node = .{ .Tuple = post_exprs }, .ty = expr.ty };
            },
            .Call => |tuple| {
                switch (tuple.f.node) {
                    .Identifier => |ident| {
                        ...
                        if (std.mem.eql(u8, ident, "return")) {
                            node = .{ .Return = try box(allocator, e) };
                        }
            ...
            .Function => |tuple| {
                var exprs = try allocator.alloc(PostAst, 0);
                for (tuple.exprs) |e| {
                    const post = try PostAst.fromAst(allocator, e);
                    exprs = try push(PostAst, allocator, exprs, post);
                }
                return .{
                    .node = .{
                        .Function = .{
